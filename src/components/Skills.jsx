 ...
 * in a centered grid design inside category cards on a horizontal carousel.
 * Rendered in App.jsx.
 */
 ...
 * @param {string} props.skillName - Name of the technology.
...
/**
 * Skills Component
 * Displays technical proficiencies in sharp domain cards with centered grid tiles.
 *
  ...
  return (
    <section id="skills" className="skills-section">
      <div className="container">
        ...
        {/* Desktop Domain Categories Carousel (>768px) */}
        <div className="skills-desktop-carousel">
          <Carousel ariaLabel="Technical Skills & Capabilities" className="skills-carousel">
            {skillsData.map((category, idx) => (
              ...
              >
                ...
                {/* Skills Grid: Logo centered inside grid cell with skill name below */}
                <div className="skills-grid-inside">
                  {category.items.map((skill) => (
                    <div key={skill.name} className="skill-grid-cell">
                      <div className="skill-grid-icon-wrap" title={skill.name}>
                        <TechLogo skillName={skill.name} size={24} />
                      </div>
        ...
        {/* Mobile Apple iOS-Style Segmented Control & Squircle Domain Card (<768px) */}
        <div className="skills-mobile-segmented">
          ...
          <div className="ios-segmented-control" role="tablist" aria-label="Skills Domain Filter">
            {domainTabs.map((tab) => (
              <button
                ...
                type="button"
                role="tab"
                aria-selected={activeDomainIdx === tab.id}
                className={`ios-segment-btn ${activeDomainIdx === tab.id ? 'is-active' : ''}`}
                onClick={() => setActiveDomainIdx(tab.id)}
              >
                <span>{tab.label}</span>
              </button>
            ))}
          ...
          {/* Active Domain Inset Grouped Card */}
          <div className="ios-skills-card">
            <div className="skills-card-header">
              <div>
                <h3 className="skills-category-title">{activeCategory.category}</h3>
                <div className="skills-category-subtitle">{activeCategory.subtitle}</div>
              </div>
              <div className="skills-icon-wrap">
            ...
            {/* 3-Column Squircle Icon Grid */}
            <div className="ios-squircle-grid">
              {activeCategory.items.map((skill) => (
                <div key={skill.name} className="ios-squircle-cell">
                  <div className="ios-squircle-icon-wrap" title={skill.name}>
                    <TechLogo skillName={skill.name} size={26} />
                  </div>
                  <span className="ios-squircle-name">{skill.name}</span>
                </div>
              ))}
            </div>
          </div>
